...
from .utils import get_console_id
...
class Console(Terminal, Color):
    ...
    def __init__(self, text: str = '', fore_color=Fore.WHITE, back_color=Back.BLACK, text_style=Style.NORMAL,
    ...
    def new_line(self, count=1, **kwargs):
        """
        Make a new line
    ...
    def get_input(self, text: str = '', to_type=str,
                  is_secret=False, exit_code=None, stream=None, commands=None,
                  **kwargs):
        fore_color = kwargs.get('fore_color', self.fore_color)
        back_color = kwargs.get('back_color', self.back_color)
        text_style = kwargs.get('text_style', self.text_style)
    ...
    def clear_console(self, wait_time_before=0, wait_time_after=0):
        self.sleep_(wait_time_before)
        os.system('cls' if os.name == 'nt' else 'clear')
        self.sleep_(wait_time_after)

    @staticmethod
    def sleep_(secs: float):
        sleep(secs)
