...
class TerminalRect:
    ...
    def move_to(self, posx, posy):
        if os.name == 'nt':
            self.refresh()
            win32gui.MoveWindow(self.console_id, posx, posy, self.width, self.height, True)


class Terminal:

    def __init__(self, title: str = win32api.GetConsoleTitle(), console_id=0, *args, **kwargs):
    ...
    def validate_size(columns, lines):
        ...
        if lines <= 0:
            raise LessThanZeroException("Terminal lines cannot be less than or equal to zero!")
