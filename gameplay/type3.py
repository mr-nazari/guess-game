...
from .utils import get_input_from_user, get_range_of_numbers, user_number_check, get_console_position


def start(console: Console):
    console.write_line('About: In this game, A random number selected for player1 and\n'
                       'another random number selected for player2.\n'
                       'Whoever responds faster wins (*^-^*)', count=2,
    ...
    guess_count = 1
    while True:
        guess_number_player1 = get_input_from_user(console, 'Player1 | '+str(guess_count))
        if user_number_check(console, guess_number_player1, random_number_player1, guess_count):
            break
        guess_number_player2 = get_input_from_user(console, 'Player2 | '+str(guess_count), space=40)
        if user_number_check(console, guess_number_player2, random_number_player2, guess_count, space=40):
            break
