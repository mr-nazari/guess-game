...
def get_input_from_user(console, guess_count, space=0):
    while True:
        is_value_None = False
        try:
            guess_number = console.get_input(
                space * " " + f"[{guess_count}] Enter your guess: ",
                int,
                exit_code="exit",
...
def get_range_of_numbers(console):
    while True:
        try:
            ...
            console.write_line(
                "Wrong range. Inputs must be numbers...", fore_color=Fore.RED, count=2
            )
        else:
...
def user_number_check(console, guess_number, random_number, guess_count, space=0):
    if random_number == guess_number:
        for i in range(300, 800, 20):
            Beep(i, 100)
        console.write_line(
            space * " " + f"You win! In {guess_count} moves...",
            count=2,
